...
Works WITHOUT tyrant module (shows card IDs instead of names)
...
Main Features:
  ...
  • Buyback Functions - Cards from Buyback store ⭐ NEW
...
class TyrantCommander:
    ...
    def get_player_info(self):
        ...
        # card slots calculate (only user_cards, buyback_data not relevant)
        ...
        if current_raids and raid_info:
            for raid_id, raid_data in raid_info.items():
                if raid_id in current_raids:
                    ...
                    print(f"  Attack Damage: {own_damage:,}")
    ...
    def calculate_max_packs(self):
        """
        Berechnet maximale Anzahl an packagesn die purchased werden can
    ...
    def get_profile(self, user_id):
        """
        Gets profile of another player
    ...
    def update_xmls(self):
        ...
        for filename, url in files_to_download:
            ...
            try:
                ...
                # 404 is OK for cards_section_22.xml
    ...
    def get_decks(self, user_id=None):
        ...
        try:
            ...
            user_decks = self.init_data.get('user_decks', {})
            # IMPORTANT: Use 'active_deck' and 'defense_deck' (without _id)!
            ...
            for deck_id, deck in user_decks.items():
                # Deck name (same as in currentdecks.txt)
    ...
    def update_deck(self):
        ...
        try:
            ...
            # Show available slots with kompletten Decks
            user_decks = self.init_data.get('user_decks', {})
            active_deck = str(self.init_data.get('user_data', {}).get('active_deck', ''))
            defense_deck = str(self.init_data.get('user_data', {}).get('defense_deck', ''))
            ...
            # Cards as JSON object: {"card_id": "count"}
            ...
            result = self.api.call('setDeckCards',
                                  ...
                                  cards=cards_json,
                                  activeYN='0')  # Not automatically set as Attack
    ...
    def _load_card_data(self):
        ...
        for section in range(1, 23):
            ...
            try:
                # ── Quelle bestimmen: lokal preferred, sonst HTTP ──
                ...
                count = 0
                for unit in root.findall('.//unit'):  # .// = deep search in all levels
                    # Required fields check – unit skip if as missing
                ...
                # Section 22 is optional - 404 is OK
    ...
    def _classify_fusion(self, card_name):
        ...
        Gibt den Gruppen-Index back, oder -1 wenn noe Gruppe passt.
    ...
    def salvage_base_epics_keep_x(self, keep_count=1, silent=False):
        ...
        Args:
            ...
            silent: Wenn True, reduzierte Output (für Workflows)
        ...
        if not silent:
            print(f"{'card':<25} {'Owned':>8} {'Keep':>10} {'Salvage':>10}")
            print("─" * 60)
        ...
        for item in to_salvage:
            ...
            total_sp += item['salvage'] * 5  # Base Epics give 5 SP
    ...
    def _load_card_data_with_rarity(self):
        ...
        for section in range(1, 23):
            ...
            try:
                ...
                for unit in root.findall('.//unit'):
                    ...
                    # Fusion-Rezept: What is its next Upgrade?
                    upgrade_id = None
        ...
        def get_fusion_tier_by_position(name):
            ...
            - Tier 0: Only Basis (no Rezept, wird aber verwendet)
    ...
    def get_inventory(self):
        ...
        try:
            ...
            for idx, (group_name, _) in enumerate(FUSION_GROUPS):
                ...
                owned_lines.append("")          # empty line between Gruppen
            ...
            # Summary
            print(f"\n=== SUMMARY ===")
            print(f"✓ ownedcards.txt: {len(owned_lines)} lines")
    ...
    def list_buyback_cards(self):
        """
        Lists all Cards in Buyback store with details and costs
        """
        buyback_info = self.get_buyback_info()
        
        if not buyback_info:
            print("\n❌ No cards in Buyback store")
            return
        ...
        print(f"cards in store: {len(buyback_info)}")
        print()
    ...
    def buyback_card(self, card_name_or_id, quantity=1):
        ...
        try:
            ...
            if quantity == 0:
                ...
                print(f"✗ Only {available}x {info['name']} available (you wanted {quantity}x)")
                return None
            ...
            # API-Call
            # Note: Based on the pattern the API could either card_id and quantity
            # or expect only card_id. We try both.
            try:
    ...
    def _find_buyback_card_id_by_name(self, card_name):
        ...
        Returns:
            card_id als int oder None if not found
        """
        buyback_info = self.get_buyback_info()
        
        # Remove -1 Suffix if present (Buyback is always Level 1)
        ...
        # Teilstring-Suche (if exact not found)
        matches = []
        for card_id, info in buyback_info.items():
    ...
    def buyback_multiple(self, rarity_filter=None, max_sp=None):
        """
        Buys multiple Cards aus dem Buyback-Store back
        
        Args:
            rarity_filter: Only Cards of this rarity buy back (1-4, None = alle)
            max_sp: Maximale SP, die spent werden sollen (None = alle availablen)
        """
        buyback_info = self.get_buyback_info()
        ...
        if not filtered:
            if rarity_filter:
                print(f"\n❌ None {RARITY_NAMES.get(rarity_filter, 'Unknown')}-cards in Buyback storetore")
            else:
    ...
    def buyback_by_names(self, card_names, max_sp=None):
        """
        Buys multiple Cards anhand einer Namensliste back
        
        Args:
            card_names: Liste von Card names oder comma-separated string
            max_sp: Maximale SP (None = alle availablen)
        """
    ...
    def buy_packs(self, count):
        """
        Buys multiple packages nacheinander mit Delay.

        Args:
            count: Number of to buy packages

        Returns:
            (purchasede: int, all_new_cards: list)
        """
        print(f"\n{'='*50}")
    ...
    def shop_salvage_workflow(self, pack_count, salvage_base_epics=False, keep_base_epics=1):
        ...
        Args:
            pack_count: Number of to buy packages
    ...
    def auto_claim_daily_bonus(self):
        ...
        try:
            ...
            # If daily_bonus_time is in the future, Cooldown is active
            ...
            if result and result.get('result') == True:
                ...
                # Show reward - versuche card name to load
                if 'reward' in result:
    ...
    def claim_daily_bonus(self):
        ...
        try:
            ...
            if result and result.get('result') == True:
                ...
                print("✗ Error while claiming Daily Bonus")
                if result:
    ...
    def export_data_to_json(self, filename=None):
        ...
        Saved in SCRIPT_DIR (data folder)
        
        Args:
            filename: Filename (default: tyrant_data_TIMESTAMP.json)
        """
        # Refresh data to get current state
        self.initialize(verbose=False)
        ...
        # Make sure that file in SCRIPT_DIR is saved
    ...
    def _build_single_card_info(self, card_name_or_id, build_count=1):
        ...
        Args:
            ...
            build_count: Number of times this card should be built (für SP costs)
        ...
        try:
            ...
            # Load card data with all upgrade levels AND fusion_tier Info
            print("\n⏳ Loading card data...")
            ...
            if card_name_or_id.isdigit():
                ...
                # Search for name - TAKE LEVEL 6 as standard
                search_name = card_name_or_id.lower().replace('-6', '').replace('-', '').strip()
                # Remove any trailing digits from search_name
                import re
                search_name = re.sub(r'\d+$', '', search_name).strip()
                
                # PASS 1: Exact match for Level 6
                for cid, info in card_data.items():
                    if isinstance(info, dict):
                        full_name = info.get('name', '')
                        level = info.get('level', 1)
                        
                        # Extract base name without level suffix
                        # "Airship-6" → "airship"
                        name_lower = full_name.lower()
                        base_name = re.sub(r'-\d+$', '', name_lower).strip()
                        
                        if search_name == base_name and level == 6:
                            target_id = cid
                            target_name = full_name
                            target_level = 6
                            break
                    elif isinstance(info, str):
                        # String format: "Name-Level"
                        name_lower = info.lower()
                        base_name = re.sub(r'-\d+$', '', name_lower).strip()
                        
                        if search_name == base_name and '-6' in info:
                            target_id = cid
                            target_name = info
                            target_level = 6
                            break
                
                # PASS 2: Partial match for Level 6 (only if no exact match)
                if not target_id:
                    for cid, info in card_data.items():
                        if isinstance(info, dict):
                            full_name = info.get('name', '')
                            ...
                            if search_name in base_name and level == 6:
                                target_id = cid
                                target_name = full_name
                                target_level = 6
                                break
                        elif isinstance(info, str):
                            base_name = re.sub(r'-\d+$', '', info.lower()).strip()
                            if search_name in base_name and '-6' in info:
                                target_id = cid
                                target_name = info
                                target_level = 6
                                break
                
                # PASS 3: Exact match for Level 1
                if not target_id:
                    for cid, info in card_data.items():
                        if isinstance(info, dict):
                            full_name = info.get('name', '')
                            ...
                            if search_name == base_name and level == 1:
                                target_id = cid
                                target_name = full_name
                                target_level = 1
                                break
                            ...
                            base_name = re.sub(r'-\d+$', '', info.lower()).strip()
                            if search_name == base_name and '-1' in info:
                                target_id = cid
                                target_name = info
                                target_level = 1
                                break
                
                # PASS 4: Partial match for Level 1
                if not target_id:
                    for cid, info in card_data.items():
                        if isinstance(info, dict):
                            full_name = info.get('name', '')
                            ...
                            if search_name in base_name and level == 1:
                                target_id = cid
                                target_name = full_name
                                target_level = 1
                                break
                            ...
                            base_name = re.sub(r'-\d+$', '', info.lower()).strip()
                            if search_name in base_name and '-1' in info:
                                target_id = cid
                                target_name = info
                                target_level = 1
                                break
            ...
            if base_target_id not in recipes:
                ...
                for check_level in range(1, 7):  # Check levels 1-6
                ...
                if highest_owned_level >= target_level:
                    ...
                    if lower_levels_available:
                        ...
                        for lvl in range(best_lower['level'], target_level):
                            key = (rarity, lvl)
                            if key in sp_costs:
                                cost = sp_costs[key]
                                total_cost += cost
                        ...
                        current_level = best_lower['level']
                ...
                for lvl in range(highest_owned_level, target_level):
                    key = (rarity, lvl)
                    if key in sp_costs:
                        cost = sp_costs[key]
                        total_cost += cost
                ...
                print(f"\n📊 Upgrade Path:")
                print(f"   {target_name}-{highest_owned_level} → {target_name}-{target_level} ({total_cost} SP)")
                
                # Upgrade without confirmation
                print(f"\n⏳ Upgrading {target_name}-{highest_owned_level} → {target_name}-{target_level}...")
                
                current_id = highest_owned_id
                current_level = highest_owned_level
                ...
                print(f"✅ {target_name}-{target_level} successfully upgraded!")
                return True
            ...
            def calculate_actual_upgrade_costs(card_id, multiplier=1, depth=0):
                ...
                # Calculate upgrade costs from best_level to needed_level
    ...
    def _find_best_card_in_inventory(self, base_id, target_level, card_data, used_cards=None):
        ...
        Returns:
            ...
            - card_id: ID der besten found card
            - current_level: Level der found card
    ...
    def _execute_build(self, target_name, target_level, base_target_id, card_data, recipes, sp_costs):
        ...
        try:
            ...
            if current_level < target_level:
                # (silent upgrade - no detailed output)
            ...
            print(f"✅ {target_name}-{target_level} successfully built!")
            return True
    ...
    def _build_card_recursive(self, card_id, card_data, recipes, sp_costs, used_cards=None):
        ...
        try:
            ...
            # KRITISCH: Check FIRST if card is already present in inventory
            if not self.init_data:
                self.initialize()
            ...
            if base_id in NEOCYTE_CORE_IDS:
                print(f"  ⚠ WARNING: Building {card_name} uses Neocyte Cores")
    ...
    #   show_dominion_fusions()    - Zeigt available Fusionen
    # ==============================================================
    
    def reset_dominion(self, dominion_card_id=None):
        """
        [LEGACY FUNCTION - Nicht mehr im Menü]
        Resettet a Dominion back auf die Basis-Versionen
        ...
        Level 1 versions (50001, 50238) existieren nicht im Spiel!
        
        Returns:
            bool: True wenn erfolgreich, False sonst
        """
        try:
            # ALWAYS reload to have current data
            self.initialize()
    ...
    def _calculate_fusion_path(self, current_id, target_id, card_data):
        """
        Berechnet den Fusion Path von current_id zu target_id
    ...
    def _execute_fusion_path(self, path, card_data):
        """
        Führt einen kompletten Fusion Path aus (Upgrades + Fusionen)
    ...
    def build_dominion_autobuild(self, dominion_card_id=None, target_level=None):
        ...
        Features:
        - Zeigt current Status (Alpha + Nexus)
        - Zeigt alle availablen Final tiers (Tier 4 Alpha, Tier 3 Nexus)
        - Berechnet automatisch Fusion Path inkl. Upgrades
        ...
        try:
            ...
            if True:
                ...
                if not available_dominions:
                    print("✗ None Final tiers-Dominions found")
                    return False
                ...
                # This is its TARGET Dominion, not its current!
            ...
            print(f"   Target:    {target_name} (Tier {target_tier})")
            ...
            if not fusion_path:
                ...
                # Confirm reset
                if not confirm_action(f"\n{current_name} backsetzen und dann {target_name} ?"):
                    print("Canceled")
                    return False
    ...
    def show_dominion_fusions(self, card_id=None):
        """
        [LEGACY FUNCTION - Nicht mehr im Menü]
        Zeigt available Dominion Fusionen für eine Card
        ...
        try:
            ...
            if card_id is None:
                ...
                print("- Within a tier use UPGRADE (upgradeCard)")
                print("="*60)
                return
            
            # Show fusions for specific card
...
def input_with_esc(prompt, allow_empty=False):
    ...
    if os.name == 'nt':
        ...
        # Unix/Linux/Mac - use normal input() with readline
        import readline
        try:
            result = input()
...
def generate_settings_from_response():
    ...
    print("Paste data and press Enter twice when done:")
    lines = []
    while True:
        line = input()
...
def interactive_menu():
    ...
    # Search for settings_*.json files
    import glob
    ...
    while True:
        if settings_files:
            choice = input(f"\nSelect (1-{len(settings_files)}, 998, 999): ").strip()
        else:
            choice = input(f"\nSelect (998 or 999): ").strip()
    ...
    while True:
        ...
        print(" 5. Export Inventory (ownedcards.txt)")
        ...
        if choice == "1":
            ...
            filename = input_with_esc("Filename (default='guild_decks', ESC=Cancel): ")
            if filename is None:
                continue
            ...
            # Make sure that .txt ending is present
            if not filename.endswith('.txt'):
                filename = filename + '.txt'
            # Use current directory (SCRIPT_DIR)
            ...
            # Base Epics salvage (keep X)
            try:
                keep_input = input_with_esc("Number of Base Epics to keep per card (default=20, ESC=Cancel): ", allow_empty=True)
                if keep_input is None:
                    continue
                
                # Empty input = default 20
                if not keep_input or keep_input.strip() == "":
                    keep = 20
                    print(f"✓ Using default: {keep}")
                else:
                    keep = int(keep_input)
            ...
            # Workflow: Buy + Commons + Rares + Base Epics salvage
            try:
                # Calculate maximum package count
                ...
                pack_input = input_with_esc(f"Number of packages (default={max_packs}, ESC=Cancel): ", allow_empty=True)
                if pack_input is None:
                    continue
                
                pack_input = pack_input.strip()
                
                # Empty input = automatically calculated maximum
                if not pack_input:
                    n = max_packs
                    print(f"✓ Using calculated maximum: {n} Packages")
                else:
                    n = int(pack_input)
                
                # Ask for keep-value for Base Epics
                keep_input = input_with_esc("Number of Base Epics keep per card (default=20, ESC=Cancel): ", allow_empty=True)
                if keep_input is None:
                    continue
                ...
                if n <= 0:
                    ...
                    if confirm_action(f"Anyway {n} Buy {n} packages + Salvage Commons/Rares/Base Epics?"):
                        commander.shop_salvage_workflow(n, salvage_base_epics=True, keep_base_epics=keep)
                    else:
                        print("Canceled")
                else:
                    if confirm_action(f"Buy {n} packages + Salvage Commons/Rares/Base Epics (keep {keep})?"):
                        commander.shop_salvage_workflow(n, salvage_base_epics=True, keep_base_epics=keep)
                    else:
                        print("Canceled")
            except ValueError:
            ...
            try:
                ...
                # Check if input is a number (ID) or name
                ...
                qty_input = input_with_esc(f"Quantity (default={info['number']}, ESC=Cancel): ", allow_empty=True)
